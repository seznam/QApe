...
describe('ActionsHandler', () => {
    ...
    it('can execute action', async () => {
        ...
        let Action = jest.fn().mockImplementation(() => actionInstance);
        let element = 'element';
        actionsHandler._getAction = jest.fn().mockReturnValue(Promise.resolve({ Action, element }));
        actionsHandler._actionsHelper = {
            waitForReadyState: jest.fn().mockReturnValue(Promise.resolve()),
        };

        await actionsHandler.execute(instance, actionId, actionConfig);

        expect(actionsHandler._actionsHelper.waitForReadyState).toHaveBeenCalledWith(instance.page);
        expect(actionsHandler._getAction).toHaveBeenCalledWith(instance, actionId, actionConfig);
        expect(Action).toHaveBeenCalledWith(
            actionsHandler._config,
            actionsHandler._actionsHelper,
            actionConfig
        );
    ...
    it('can get action by ID and actionConfig', async () => {
        ...
        actionsHandler._actionsHelper = {
            getElement: jest.fn().mockReturnValue(Promise.resolve(element)),
        };
        ...
        expect(actionsHandler._actionsHelper.getElement).toHaveBeenCalledWith(instance.page, actionConfig);
    });
    ...
    it('can get action by ID and without actionConfig', async () => {
        ...
        class DummyAction {}
        actionsHandler._actions = { id: DummyAction };
        actionsHandler._getAvailablePageActions = jest.fn().mockReturnValue(
            Promise.resolve([
                {
                    Action: DummyAction,
                    element,
        ...
        expect(results).toEqual({ Action: DummyAction, element });
        expect(actionsHandler._getAvailablePageActions).toHaveBeenCalledWith(instance.page, [actionId]);
    });
    ...
    it('can get random action', async () => {
        ...
        expect(results).toEqual({ Action: DummyAction, element });
        expect(actionsHandler._getAvailablePageActions).toHaveBeenCalledWith(instance.page, ['id']);
    });

    it('can initialize actions helper', () => {
        actionsHandler._initActionsHelper();

        expect(actionsHandler._actionsHelper instanceof ActionsHelper).toEqual(true);
    });
});
