...
describe('Reporter', () => {
    ...
    it('can initialize all defined reporters', () => {
        ...
        expect(reporter._initReporterFromClass).toHaveBeenCalledWith(SpinnerReporter);
        expect(reporter._initReporterFromClass).toHaveBeenCalledWith(ConsoleReporter);
        expect(reporter._initReporterFromClass).toHaveBeenCalledWith(FileReporter);
        expect(reporter._initReporterFromString).toHaveBeenCalledWith(reporters[3]);
        expect(reporter._initReporterFromClass).toHaveBeenCalledWith(reporters[4]);
    });
    ...
    it('can emit an event for all defined reporters', () => {
        ...
        expect(reporterInstance.emit).toHaveBeenCalledWith(eventName, data);
    });
