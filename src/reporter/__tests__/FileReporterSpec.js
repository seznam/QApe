...
describe('FileReporter', () => {
	...
	it('can handle failing not reproduced scenario end', () => {
		...
		expect(reporter._isFailureReported)
			.toHaveBeenCalledWith(eventData.scenario);
		expect(reporter._results).toEqual([{
			scenario: eventData.scenario,
			errors: eventData.errors
		}]);
		expect(reporter._getScenarioName)
			.toHaveBeenCalledWith('-not-reproduced');
		expect(reporter._logFile)
			.toHaveBeenCalledWith(
				eventData.scenario,
				eventData.errors,
